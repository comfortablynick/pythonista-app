#!/usr/bin/env python3
"""Downloads and opens a Gist from a URL in the clipboard.

Works with "raw" and "web" gist URLs, but not with non-Python files. If a file already exists, a dialog is shown that asks whether the new file should be renamed automatically.
"""
import clipboard
import editor
import console
...
def download_gist(gist_url: str) -> tuple:
    """Returns a tuple of filename and content."""
    console.show_activity()
    urlsplitted = urlsplit(gist_url)
    raw_match = re.match("http(s?)://gist.githubusercontent.com/.+/raw/", gist_url)
    # TODO: list domains here and use urlsplit
    netlocs = ("raw.githubusercontent.com",)
    if raw_match or urlsplitted.netloc in netlocs:
    ...
    try:
        for num, (filename, content) in enumerate(download_gist(gist_url), start=1):
            if os.path.isfile(filename):
                i = console.alert(
                    "File exists",
                    f'A file named "{filename}" already exists in your library.',
                    "Auto Rename",
                )
                if i == 1:
